...
import { titles, formatCurrency, getDaysUntil, formatDate } from "@/data/mockData";
import { cn } from "@/lib/utils";

const Titles = () => {
  const totalGoal = titles.reduce((sum, t) => sum + t.revenue_goal, 0);
  const totalBooked = titles.reduce((sum, t) => sum + t.revenue_booked, 0);
  const overallPercent = Math.round((totalBooked / totalGoal) * 100);
                ...
                <div className="mt-4 pt-4 border-t border-border">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium tabular-nums">{formatCurrency(title.rates.full)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">½ Page</span>
                      <span className="font-medium tabular-nums">{formatCurrency(title.rates.half)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">¼ Page</span>
                      <span className="font-medium tabular-nums">{formatCurrency(title.rates.quarter)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Spread</span>
                      <span className="font-medium tabular-nums">{formatCurrency(title.rates.spread)}</span>
                    </div>
                  </div>
                </div>
              </div>
