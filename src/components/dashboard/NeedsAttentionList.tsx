...
import { Account, formatDate } from "@/data/mockData";
...
export function NeedsAttentionList({ accounts }: NeedsAttentionListProps) {
  const getReasonIcon = (account: Account) => {
    ...
    if (account.waffling_score > 50) return TrendingDown;
  ...
  const getReason = (account: Account) => {
    ...
    if (account.waffling_score > 50) return `Waffling score: ${account.waffling_score}%`;
